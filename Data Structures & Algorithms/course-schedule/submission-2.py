...
class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        # 1. convert into a adj list
        #    pre: [courses]
        # 2. do dfs on each course list
        # 3. if we met a already visited 
        ...
        def dfs(course):
            ...
            if course in visiting: # loop
                return False
            
            if len(adj[course]) == 0:
                visited.add(course) # valid
                return True

            visiting.add(course) # 2
            for c in adj[course]:
                if not dfs(c): # if true means the course is reachable
                   return False
